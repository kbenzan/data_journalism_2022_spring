...
most_recent <- wvu_campus_incidents %>% 
  arrange(desc(date))

suspicious_event <- most_recent %>% 
...
incidents_previous_30 <- paste("Since", format(most_recent$date, format="%B %d"), "when the most recent incident", format(most_recent$title), direction, "happened,"  " incidents of that kind have occurred in the previous 30 days", sep="")
```
