...
maryland_zips_yyymmdd <- total_vacs_by_zip %>% 
  filter(ZIP_CODE > 20601 & ZIP_CODE < 21930)

non_maryland_zips_yyyymmdd <- total_vacs_by_zip %>% 
  filter(ZIP_CODE < 20601 | ZIP_CODE > 21930)

# today's date
current <- maryland_zips_yyymmdd %>% summarize(max(date))
...
maryland_zips_yyymmdd <- maryland_zips_yyymmdd %>% 
  mutate(across(ZIP_CODE, as.character))

# join
md_zips_vacs <- md_county_population %>% left_join(maryland_zips_yyymmdd, by="ZIP_CODE")

# map
ggplot() +
  geom_sf(data=md_zips_vacs, aes(fill=cases)) +
  scale_fill_viridis_c(option = "plasma", trans = "sqrt") +
  theme_void() +
  labs(title = paste("People Fully Vaccinated as of ", current[[1]], sep=""))
```
