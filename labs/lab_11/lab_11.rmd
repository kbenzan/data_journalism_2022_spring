...
wv_county_population <- wv_county_population %>%
  mutate(COUNTY = toupper(str_remove_all(COUNTY,", WEST VIRGINIA"))) %>%
...
wv_population_mining <- wv_county_population %>% inner_join(amount_per_county, by=c("COUNTY" = "project_county_name")) %>% select(COUNTY, population, geometry, total_amount, total_number)
...
# map
write_csv(wv_population_mining, "data/wv_population_mining.csv")

# table
view(wv_population_mining)
  
```

**A1.** 
https://datawrapper.dwcdn.net/1cFJX/1/

Based on the map, central West Virginia got the most loans in this industry. There seems to be a diagonal line through central West Virginia that shows the counties with the most loans.

Based on the table, HARRISON county got the highest total loan amount. total amount per 100K population = $14,607,650.03; total amount of loans (not population adjusted) = $9,919,762.98; number of loans = 59; population = 67908
