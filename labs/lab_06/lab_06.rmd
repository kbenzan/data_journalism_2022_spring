...
**A1** Suburban Washington Region and Montgomery County accounted for the largest percentage of international migration for the entire time period
...
**A3** Calvert County and Sothern Maryland Region recorded the largest percentage changes. 	
...
**A6** 14 regions and jurisdictions had net positive migration for those three years.

```{r}
# Write the code to produce your answer here
domestic_migration %>% 
  filter(`2016_2017` > 0 & `2017_2018` > 0 & `2018_2019` > 0) %>% 
  select(`2016_2017`, `2017_2018`, `2018_2019`, `State/Jurisdiction (7/1)`)
```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** With few exceptions, the data is filled with counties. Then two regions - Southern Maryland and Upper Eastern Shore - stand out. The list contains only three of five counties from Upper Eastern Shore; the list then contains two of three counties from Southern Maryland. The rest of the list incorporates northern and central counties, and approximately half of Maryland's counties are included. It also appears that the larger counties have similar numbers as the regions. For example, Anne Arundel. Frederick even has a net positive larger than the two regions - almost twice as large. 
