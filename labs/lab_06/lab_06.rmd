...
international_migration <- read_csv("data/tabula-md-statistical-handbook.csv", skip=2, col_names = c("State/Jurisdiction (7/1)", "2010_2011", "2011_2012", "2012_2013", "2013_2014", "2014_2015", "2015_2016", "2016_2017", "2017_2018", "2018_2019", "2010_2019"))
```
...
**A1** Suburban Washington Region accounted for the largest percentage of international migration for the entire time period

```{r}
# Write the code to produce your answer here
international_migration <- international_migration %>% mutate(percent_total = 100*(`2010_2019`/198996))
...
**A2** The data in A1 represents that the most international migration took place in Suburban Washington Region within Maryland. About 63% of immigrants decided to relocate to that region (Frederick, Montgomery and Prince George's counties). This data makes sense due to the abundance of international business that occurs in D.C. 
...
**A3** Calvert County recorded the largest percentage change: -76.923077. 	

```{r}
# Write the code to produce your answer here
international_migration <- international_migration %>% mutate(percent_change = 100*((`2017_2018`-`2016_2017`)/`2016_2017`))
...
**A4** A natural disaster is my best guess as to why the international migration decreased by that much. Calvert County is a southern county with a coast. Hurricanes and coastal storms could divert immigrants from settling in Calvert.
...
```{r}
# Write the code to load the CSV file here
domestic_migration <- read_csv("data/tabula-md-statistical-handbook-domestic.csv", skip=2, col_names = c("State/Jurisdiction (7/1)", "2010_2011", "2011_2012", "2012_2013", "2013_2014", "2014_2015", "2015_2016", "2016_2017", "2017_2018", "2018_2019", "2010_2019"))
```
...
**A5** Montgomery County saw the largest net decrease in domestic migration

```{r}
# Write the code to produce your answer here
domestic_migration <- domestic_migration %>%  mutate(percent_change = 100*((`2018_2019`-`2010_2011`)/`2010_2011`))
...
**A6** 18 regions and jurisdictions had net positive migration for those three years.

```{r}
# Write the code to produce your answer here
domestic_migration <- domestic_migration %>%  mutate(net_2017_2019 = `2016_2017`+`2017_2018`+`2018_2019`)

domestic_migration %>% 
  filter(str_detect(net_2017_2019, "^-"))
```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** This data is interesting because those regions with a net positive contain such lower numeric figure than those with a net negative. For instance, Frederick - the county with the largest net positive - recorded 8,536, whereas Suburban Washington Region - the region with the largest net negative - recorded -48,302. That being said, a region is larger than a county. But if we compare Frederick with the largest net negative country, Frederick still recorded such a lower numeric figure that Baltimore (-30,495). Also, the counties surrounding D.C. seemed to record net negatives, whereas the counties further out from D.C. recorded net positives. This pattern is contrary to international migration, where the counties near D.C. recorded net positives. 
