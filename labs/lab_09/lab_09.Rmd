...
westva_ppp_april20 %>%
  ...
  geom_text(aes(x=date_approved, y=total_loans, label=total_loans, fontface=2, color='red'))
...
loans_per_people <- loans_per_people %>% 
  mutate(
    per_100000 = total_ppp_loans/100000
  )
...
top10_loans_per_people %>%
  ggplot() +
  geom_bar(aes(x=reorder(state, per_100000), weight=per_100000, stat="identity", fill="lightblue")) +
  coord_flip() +
  labs(
    title="The States with the Most Loans Per 100,000 People",
