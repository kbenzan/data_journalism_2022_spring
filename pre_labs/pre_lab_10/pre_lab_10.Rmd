...
https://datawrapper.dwcdn.net/6uDnS/2/
...
The code block is making a new dataframe, named md_counties, by grouping by counties and then counting how many loans per county. The output is a two-column table with the counties and the count for each county.
...
**Answer** Describe what the code below is doing and the output.
This code block is updating the previous dataframe by filtering the count. It is eliminating any above 6. The output is the same list, although without any weird values.
...
The new fips_code column contains the 5-digit code for each county. All start with "24."
...
I see md_counties + md_fips = md_counties_with_fips. I see the fips code column, along with the count and project_county_name columns.
...
Now the map is confined to Maryland.
...
**Answer** Describe the changes from the previous map
Now when you hover over a particular county, you see the county names and the number of PPP loans for that county. The number is more easily readable. 
...
https://datawrapper.dwcdn.net/AKquI/1/
