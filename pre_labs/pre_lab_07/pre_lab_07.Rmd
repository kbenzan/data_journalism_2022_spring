...
**Answer** I first see a list of the industry classification, and then further down I see tables regarding NAICS numbers.
...
**Answer** I see HTML language with a long list of tags. 
...
**Answer** The output describes a HTML document with a "head" tag and a "body" tag. 
...
**Answer** I see a table with "list of 2" as values. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** I see similar HTML language as in "view source." 
...
**Answer** I see 6 different tibbles. The second tibble contains both sector and description.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** I see a table of each of the 6 tibbles. 
...
**Answer** The hidden scroll reveals a table containing the reference month implemented and date release for four periods (SIC to NAICS 2002, NAICS 2002 to NAICS 2007, NAICS 2007 to NAICS 2012, NAICS 2012 to NAICS 2017).
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** I see a dataframe of sector and description.
...
**Answer** The output reveals a tibble of the same data as the image above. That code converted that HTML data into a dataframe.
...
**Answer** This following code eliminated the last row, which contained the source of the data. The code also cleaned the names.
...
**Answer** The output shows a tibble of sectors and descriptions.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** I see a table that describes all employment for the Utilities sector. I see the 538.7 that is included in the image above.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** The output shows the code for an HTML document. It includes a "head" tag and a "body" tag. 
...
**Answer** The output shows an HTML node with the HTML code for the table (id="iag22emp1").
...
**Answer** I see a tibble with the employment, backdata and months. However, the backdata column is pretty much blank. Then the final row reads "Footnotes(p) Preliminary" in every column.
...
**Answer** I only see the employment from November 2021 (536.9). 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** I see a table of all employing for the Mining, Quarrying, and Oil and Gas Extraction sector. It includes the 536.9 that we can find above.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Same thing as the Mining, Quarrying, and Oil and Gas Extraction sector, but just for the Construction sector.
...
**Answer** I see just a list of all the sectors. 
```{r}
...
**Answer** The following code adds a value in the Environment section "list of sectors" with 10 observations.
```{r}
...
**Answer** This code is similar to print each sector, however more efficiently. Now by just saying "sector" in a loop, the output will print all sectors in the "list of sectors".
```{r}
...
**Answer** I receive an error message that says "object 'sector_list' not found." This is because our list is named "list_of_sectors".
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** This output looks very similar as the output from Task 16. What's different is the stand-in "x."
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** I see an error message: object 'sector_name' not found. We first said "x" then changed it to "sector_name."
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** I see a list counting from 1 to 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** I see a tibble that includes the sector and description with 20 observations.
...
**Answer** The output is 20 because there are 20 sectors.
...
**Answer** I see a list ranging from 1 to 20.
...
**Answer** The coded tibble gives the employment for the Mining, Quarrying, and Oil and Gas Extraction sector from November 2021.
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** I see our tibble with the naics sector and discription.
```{r}
# Define url of page we want to scrape
...
**Answer** The ouput includes the usual sector and description columns, but also a column that contains the URL for each sector.
...
**Answer** The output includes the usual sector and description columns, also a sector URL column and a sector xpath column.
...
**Answer** This tibble is the same as above, except for one difference. This tibble does NOT include "Public Administration," so there are only 19.
...
**Answer** The output contains 19 tibbles. Each tibble reveals one row for each sector.
...
**Answer** The output is a list that describes each row. For example, [1] "ROW NUMBER:1 URL: https://www.bls.gov/iag/tgs/iag11.htm XPATH://*[@id=\"iag11emp1\"]"
```{r}
...
**Answer** This code extracts the online table and spits out 19 tibbles for each sector. Although, there is some extra stuff that we'll need to clean up (i.e. Backdata).
...
**Answer** 19 tibbles pop up that show the employment for each sector for four months (Nov 2021, Dec 2021, Jan 2022, and Feb 2022).
...
**Answer** This output combines the output from Task 32, the sector number, description, URL and xpath_id.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** Now these tibbles simply incorporate the sector code, the description and November 2021 employees.
...
**Answer** Now the different tibbles from above are combined into one table with 19 rows.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** I see the employment information about the Mining, Quarrying, and Oil and Gas Extraction sector.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** I only see the unemployment rate from the Agriculture, Forestry, Fishing and Hunting sector. 
...
**Answer** I see our finished table however the number of employees for Agriculture, Forestry, Fishing and Hunting is NA. 
