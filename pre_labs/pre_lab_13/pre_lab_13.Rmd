...
**Answer** A dataframe that shows the 2016 election results, such as how each state voted and what percentage voted for Trump.
```{r}
...
**Answer** I see a positive correlation between the percentage of which businesses receive ppe and the percentage of which voted for Trump. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There isn't as clear of a correlation between the ppe amount per employee and the percentage of which voted for Trump. The blue line is flatter than above.
```{r}
...
**Answer** I see the correlation value (r) for each of the variables when comparing against pct_trump. The table puts a number to what we saw above - businesses_receiving_ppe_pct has a stronger correlation with pct_trump than ppe_amount_per_employee.
```{r}
reveal_data %>%
...
**Answer** This model output supplies both the t and p value, along with the 95% confidence interval. I see basic statistics with this data.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** I see similar statistics however this correlation includes ppe_amount_per_employee. So, we see the r value in the last time (0.221133).
```{r}
...
**Answer** We see the mean_ppp_amount_per_employee and the mean_businesses_receiving_ppe_pct for both Trump and Clinton (how they voted in the 2016 election).
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** I see the result of a t-test. t=-1.2223. The p-value was 0.2295. We also see the two means from Clinton and Trump. By these results, we can see that the two means are statistically different.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Again we see t=-3.182, p-value = 0.002643 and the two means (Clinton = 28.23810; Trump = 37.23333). Again these results can lead us to believe that the two means are statistically different.
```{r}
