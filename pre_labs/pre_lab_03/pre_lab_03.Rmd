...
**Answer** No error messages pop up.
...
**Answer** Both the "amount" field and the id column are numerical, but the "name" field is character.
```{r}
...
**Answer**: Where the franchise_name is NA there are 192,959 loans. Where the franchise_name is "Subway" there are 275 loans.
...
**Answer**: There are 135,073 loans where the forgiveness data is NA. It is the same number as the forgiveness amount.
...
**Answer**: There are 135,073 loans where forgiveness_date and forgiveness_amount is NA. It is the same number as above. I can conclude that about 135,000 companies have not had their loans forgiven. Loan forgiveness means you don't have to pay back all or some of loan. Thus, over 135,000 companies still owe money. This conclusion also is sensical because if a company doesn't have a forgiveness amount, then they shouldn't have a forgiveness date. This codeblock demonstrated that they are the same records.
...
**Answer** The largest loan is for $10,000,000 and the smallest loan is for $6. The largest loan is not suspicious to me because $10 million is the maximum amount a company can ask for. However, the $6 loan amount is suspicious to me because the purpose of the PPP is to pay payrolls and $6 won't do much to help a company with paying their employees. 
```{r}
...
**Answer** The selected columns do align. All columns in the row GETGFTD LLC read 6, and all columns in the row LEGACY SPINE AND PAIN LLC read 78. 
...
**Answer** 6 rows are in this data set.
...
**Answer** 1. The column headers are inconsistently styled (i.e. "1_id", "NAME" and "street address")
2. The amount column is stored as a character, not a number.
3. There's a row that appears twice in the dataset.
...
**Answer** x1_id is how the function changed the name of "1_id." 
...
**Answer** Error: Problem with `summarise()` column `total_amount`.
ℹ `total_amount = sum(amount)`.
x invalid 'type' (character) of argument

It appears as though total_amount is stored as character and not numeric. Therefore, R cannot understand the code written to sum up total_amount as if it were numbers.
...
**Answer** There are two duplicated rows under the business STUART JONES LLC.
...
**Answer** The new dataframe contains 5 rows now.
...
**Answer** The zip code across all 5 entries is now 21012.
...
**Answer** Two misspellings of Arnold are left.
...
**Answer** 1 misspelling remains after running this code.
...
**Answer** 0 misspellings appear in the tibble below.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
