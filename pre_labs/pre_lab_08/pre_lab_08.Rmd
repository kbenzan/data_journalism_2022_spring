...
# decennial census - sf1 (most recent year)
# acs - rolling survey (not as accurate)
# acs1 (most recent, just one year), acs3 (three years, more data)
```
...
# pl file - used for redistricting
```
...
**Answer** Somerset County and Caroline County. For Somerset, the change could be within the margin of error. For example, if the 2000 income was the minimum and the 2010 income was the maximum, then it would be a positive change between median incomes. For Caroline, it's possible to have a positive change in median income. If the 2010 income was larger than $60,858, then there would be growth. Because it's such a small decrease for both, there are chances for positive growth in median incomes. I am not confident in the decreases in each of the median incomes.

```{r}
md10 %>%
  # join the data 
  inner_join(md19, by=c("GEOID", "NAME")) %>% 
  # rename some fields that repeat
  rename(estimate2010=estimate.x, estimate2019=estimate.y) %>%
  # minimum and maximum value of the estimate using the margin of error
...
**Answer** One has an estimated median household income of less than $50,000 considering the margin of error. The zip code is 21201, and the name is ZCTA5 21201.
