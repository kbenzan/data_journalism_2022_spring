...
**Answer** The data frame lists the vaccination status by county and dosage. It also contains cumulative numbers.
...
# the date function extracts only the date from mdy_hms
# mdy_hms = month day year _ hour minute second
```
...
**Answer** The data frame only contains two columns: county and total. However, one county is named "NA." But that NA county accounted for 284 total.
...
**Answer** This tible includes all the vaccinations for "NA" county. 
...
county_vaccinations_by_date <- county_vaccinations_by_date %>%
  filter(!is.na(county))
# exclamation point --> do the opposite --> so IS NOT n/a

# what does county represent, if there are NA values?
# didn't report that information? people who don't live in MD?
# don't know what NA means 
# 1/8/21 -- 8,000 in a day ... not super plausible
```
...
**Answer** The data frame includes how many are fully vaccinated by date and for Prince George's County. The most recent 7-10 days show a steady decline in the daily number of those fully vaccinated. Those days aren't highly ranked either.
...
**Answer** The chart is showing the number of vaccinations dispersed per day from Jan 2021 to Jan 2022.
...
**Answer** Sum reads "The number of people in Prince George's who got fully vaccinated on March 6 decreased by -73.77 percent compared to the week before"
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer** Sub didn't change - "The number of people in Prince George's who got fully vaccinated on March 6 decreased by -73.77 percent compared to the week before"
...
**Answer** We see the number of cases from July 2020 to Jan 2022 for each of the Maryland counties
...
**Task** Run the following code and describe what the resulting chart shows
**Answer** This chart shows the same as above, but the y-axis is scaled logarithmically. Also, we see a similar steady increase across counties.
...
**Answer** The map shows the number of covid cases for each county as of Apr 18, 2022.
